...
class UPSampleConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1, scale_factor=4, mode='bilinear', align_corners=True):
...
class DPTHead(nn.Module):
    ...
    def __init__(
        ...
        if norm_layer == "instancenorm":
            self.norm = nn.InstanceNorm1d(dim_in, affine=False)
        ...
        if resize_method == "deconv":
            ...
            self.resize_layers = nn.ModuleList(
                [
                    UPSampleConv2d(
                        in_channels=out_channels[0], out_channels=out_channels[0], kernel_size=3, stride=1, padding=1, scale_factor=4
                    ),
                    UPSampleConv2d(
                        in_channels=out_channels[1], out_channels=out_channels[1], kernel_size=3, stride=1, padding=1, scale_factor=2
                    ),
                    nn.Identity(),
                    nn.Conv2d(
                        in_channels=out_channels[3], out_channels=out_channels[3], kernel_size=3, stride=2, padding=1
